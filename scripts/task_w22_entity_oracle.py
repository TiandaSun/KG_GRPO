...
Usage::
    ...
    python scripts/task_s12_oracle_l3.py \
        ...
        --output results/phase7/w22_entity_oracle_full_test.json
"""
...
def collect_gold_entities(kg_path: Any) -> list[str]:
    """W22 — gold ENTITY chain: ordered, deduplicated heads (idx 0) + tails
    (idx 2) from kg_path. Candidate set for the entity-threading oracle
    (mirror of collect_gold_relations)."""
    ents: list[str] = []
    seen: set[str] = set()
    if kg_path is None:
        return ents
    ...
    for triple in iterator:
        ...
        if len(triple_list) < 3:
            continue
        for idx in (0, 2):  # head, tail
            ent = str(triple_list[idx]).strip()
...
def generate_with_oracle_tools(
    ...
    for turn in range(max_turns):
        ...
        action, policy_entity, policy_relation = call
        num_tool_calls += 1

        # ---- W22: Oracle ENTITY rewrite ----------------------------------
        # Replace the policy's ENTITY argument with the edit-closest gold entity
        # (entity-threading / L_comp oracle). The policy's RELATION argument is
        # kept untouched — mirror of the S1.2 relation oracle.
        rewritten_entity = policy_entity
        oracle_chosen: str | None = None
        if policy_entity is not None and gold_entities:
            oracle_chosen = pick_oracle_relation(policy_entity, gold_entities)
            if oracle_chosen is not None:
                rewritten_entity = oracle_chosen

        kg_result = call_kg_server(action, rewritten_entity, policy_relation, kg_server_url)
        # ------------------------------------------------------------------
        ...
        tool_records.append({
            ...
            "oracle_chosen_entity": oracle_chosen,
            "entity_used": rewritten_entity,
            "policy_relation": policy_relation,
            "rewritten": (oracle_chosen is not None and oracle_chosen != policy_entity),
        })
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("results/phase7/w22_entity_oracle_full_test.json"),
    )
    parser.add_argument(
        "--save_per_sample",
        type=Path,
        default=Path("results/phase7/w22_entity_oracle_per_sample"),
    )
    parser.add_argument(
        "--save_trajectories",
        type=Path,
        default=Path("results/trajectories/phase7/w22_entity_oracle_full"),
    )
    ...
    if args.resume and ps_file.exists():
        try:
            ...
            for rec in prior:
                ...
                rewrite_counts.append(int(rec.get("num_entity_rewrites", 0)))
    ...
    for i, (_, row) in enumerate(df.iterrows()):
        ...
        gold_entities = collect_gold_entities(row["extra_info"].get("kg_path"))
        n_evaluated += 1
        ...
        per_sample.append({
            ...
            "num_entity_rewrites": int(n_rewrites),
